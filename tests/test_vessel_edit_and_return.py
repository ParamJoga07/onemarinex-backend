...
class _Base(unittest.TestCase):
    ...
    def call_for(self, vessel, *, ended=False):
        call = VesselCall(
            ...
            ended_at=NOW - timedelta(days=1) if ended else None,
        )
        self.db.add(call)
        self.db.flush()
        return call
...
class VesselEditTests(_Base):
    ...
    def test_pushing_a_departed_vessels_etd_forward_does_not_start_a_voyage(self):
        """The defect an end-to-end run caught that the tests above missed.

        Status is derived from ETD, so moving it into the future flips a
        departed vessel back to Active — and the lifecycle sync then opened a
        call because the previous one had ended. Correcting a date started a
        new voyage.

        The vessel keeps its agent here, unlike the test above: an unassigned
        vessel was already refused a call for a different reason, which is why
        that one passed while the real thing failed.
        """
