...
def get_pubs(
    ...
    query = db.query(Vendors).filter(vendor_category_text().ilike("pub"), Vendors.status == "Active")
    if port_id:
...
def get_pub_details(
    ...
    v = db.query(Vendors).filter(Vendors.id == pub_id, vendor_category_text().ilike("pub")).first()
    if not v:
...
def get_pub_qr(id: int, db: Session = Depends(get_db)):
    v = db.query(Vendors).filter(Vendors.id == id, vendor_category_text().ilike("pub")).first()
    if not v:
        raise HTTPException(status_code=404, detail="Pub not found")
